...
def main():    
    ...
    wan_usage_history_plot_using_api_key(dashboard, network_id, t0='2021-09-01T16:00:00Z', t1='2021-09-02T16:00:00Z', 
    reso = resolution_a, bandwidth_limit=bdw_limit)
    wan_usage_history_output_to_excel(dashboard, network_id, t0='2022-08-29T16:00:00Z', t1='2022-08-30T16:00:00Z', 
    reso = resolution_a)
...
def wan_usage_history_plot_using_json_file(reso, bandwidth_limit):
    ...
    file = 'uplink_20220901_1541pm.json'
...
def loss_and_latency():
    ...
    file = "loss_and_latency_20220901_1555pm.json"
...
def cpu_usage():
    ...
    file = 'load_monitor_history_20220901_1555pm.xlsx'
    dict_df = pd.read_excel(file)    
    time = dict_df.get('time')
    data = dict_df.get('data') 

    load_value = {}
    load_value['0'], load_value['1'], load_value['2'], load_value['3'], \
        load_value['4'], load_value['5'], load_value['6'], load_value['7'] = ([] for i in range(8))

    for i in data:
        i_list = json.loads(i)
        for j in i_list:
            if(j['is_click_core']) == True:
                #load_value_click_core[click_number] = load_value_click_core[click_number] + j['load_vals']
                # extend is more efficient than using '+'
                load_value[str(j['core_id'])].extend(j['load_vals']) 

    for i in load_value:
        if (len(load_value[i]) != 0):
            plt.plot(np.array(load_value[i]), linewidth = '0.3', label = 'Core id ' + i)

    plt.legend(loc='best')
    plt.show()

    # Cannot use plot_data function here due to len of time and the jason data 
    # don't match well. 
    #plot_data('CPU Usage History', load_value, time.values[0], time.values[-1], 1)
